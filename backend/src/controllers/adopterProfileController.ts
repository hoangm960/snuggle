...
export const getAdopterProfile = async (
  req: AuthRequest,
  res: Response
): Promise<void> => {
  try {
    ...
    throw new AppError('Failed to fetch adopter profile', 500);
  }
};

export const createAdopterProfile = async (
  req: AuthRequest,
  res: Response
): Promise<void> => {
  try {
...
export const updateAdopterProfile = async (
  ...
  try {
    ...
    const profile: AdopterProfile = { id: updatedDoc.id, ...updatedDoc.data() } as AdopterProfile;

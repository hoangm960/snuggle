...
export const getShelterReviews = async (req: AuthRequest, res: Response): Promise<void> => {
	try {
...
async function updateShelterTrustScore(shelterId: string): Promise<void> {
	const snapshot = await getReviewsCollection(shelterId).where("status", "==", "approved").get();

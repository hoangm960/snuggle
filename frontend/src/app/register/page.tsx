...
export default function RegisterPage() {
  ...
  const validate = () => {
    ...
    else if (password.length < 8) { setPasswordError('Your password must have at least 8 characters'); valid = false }
    if (!confirmPassword) { setConfirmError('Please re-enter your password'); valid = false }
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/auth/register', {
        ...
        body: JSON.stringify({ email, password, displayName: username, role: 'user' }),
      })
    ...
    <div className="flex flex-col min-h-screen w-full">
        ...
        <div className="hidden lg:flex items-center gap-4">
          ...
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none" style={{ flexShrink: 0 }}>
              <g clipPath="url(#clip0_reg_search)">
              ...
              <defs><clipPath id="clip0_reg_search"><rect width="16" height="16" fill="white"/></clipPath></defs>
            </svg>
            <input
              type="search"
              placeholder="Search..."
      ...
      <div className="flex flex-1">
        ...
        <div className="hidden lg:flex lg:w-[45%] xl:w-1/2 relative flex-shrink-0">
          <img src="/images/hero1.png" alt="Pets" className="absolute inset-0 w-full h-full object-cover" />
          ...
          <main className="flex-1 flex flex-col items-center justify-center px-6 py-20">
            <div className="w-full max-w-[420px]">
              ...
              <p className="text-xs text-center text-[#999999] mb-14">
                ...
                <Link href="/login" style={{ color: '#7AADA1' }} className="font-medium hover:underline">
                  Log in
                </Link>
              </p>
              ...
              <form onSubmit={handleRegister} noValidate>
                ...
                <div className="mb-9">
                  ...
                  />
                  {emailError && <p className="mt-1.5 text-xs text-[#EB4335] flex items-center gap-1"><ErrorIcon />{emailError}</p>}
                </div>

                {/* Username */}
                <div className="mb-9">
                  <label className="block text-sm font-medium text-[#333333] mb-2">User name</label>
                  <input
                    type="text"
                    value={username}
                  ...
                  />
                  {usernameError && <p className="mt-1.5 text-xs text-[#EB4335] flex items-center gap-1"><ErrorIcon />{usernameError}</p>}
                </div>
                ...
                <div className="mb-9">
                  ...
                  {passwordError && <p className="mt-1.5 text-xs text-[#EB4335] flex items-center gap-1"><ErrorIcon />{passwordError}</p>}
                  {!passwordError && <p className="mt-1.5 text-xs text-[#666666]">Your password must have at least 8 characters</p>}
                </div>

                {/* Re-enter Password */}
                <div className="mb-9">
                  <label className="block text-sm font-medium text-[#333333] mb-2">Re-enter Password</label>
                  <div className="relative">
                    <input
                  ...
                  {confirmError && <p className="mt-1.5 text-xs text-[#EB4335] flex items-center gap-1"><ErrorIcon />{confirmError}</p>}
                </div>

                {/* Terms */}
                <p className="text-xs text-[#999999] mb-10 leading-relaxed">
                  By creating an account, you agree to our{' '}
                  <a href="#" className="underline text-[#333333] hover:text-[#7AADA1]">Terms of use</a>
                  {' '}and{' '}
                  <a href="#" className="underline text-[#333333] hover:text-[#7AADA1]">Privacy Policy</a>
                </p>
                ...
                <div className="flex justify-center mb-14">
                  <button
                    ...
                    style={{ width: '140px', height: '48px', borderRadius: '16px', backgroundColor: '#111' }}
                  >
              ...
              {/* Or continue with */}
              <div className="text-xs text-[#999999] text-center" style={{ border: 'none', marginBottom: '24px' }}>
                Or continue with
              </div>
              ...
              <div className="flex flex-col gap-4">
                <button
                  ...
                  style={{ height: '48px', borderRadius: '40px', border: '1px solid #333', background: '#FFF' }}
                >
                  <GoogleIcon />
      ...
      <footer className="px-8 py-6 border-t border-[#CCCCCC] bg-white">
        <div className="flex flex-wrap gap-x-4 gap-y-2 justify-center mb-3">
          {FOOTER_LINKS.map((link) => (
            <a key={link} href="#" className="text-xs text-[#666666] hover:text-[#333333] hover:underline">{link}</a>
          ))}
        </div>

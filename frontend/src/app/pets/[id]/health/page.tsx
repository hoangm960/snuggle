...
import { ArrowLeft, HeartPulse, CheckCircle2, PawPrint, Syringe, Stethoscope, Loader2 } from "lucide-react";
		...
		<div className="min-h-screen" style={{ background: "#F9F6F2", fontFamily: "'Poppins', sans-serif" }}>
			...
			<div className="max-w-3xl mx-auto px-6 py-8">
				<div className="rounded-2xl p-6 mb-6 relative overflow-hidden" style={{ background: "linear-gradient(135deg, #7AADA1, #216959)", color: "#fff" }}>
					...
					<p style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: "28px", fontWeight: 700, marginTop: "2px" }}>{pet.name}</p>
					<p style={{ fontSize: "13px", opacity: 0.85, marginTop: "2px", textTransform: "capitalize" }}>{pet.species}</p>
					<div className="flex items-center gap-2 mt-4">
						<CheckCircle2 className="size-4" />
						<span style={{ fontSize: "13px" }}>{records.length} health records on file</span>
					</div>
				</div>
										...
										<div className="flex items-start gap-3">
											...
											<div className="flex-1">
												<div className="flex items-center justify-between">
													<p style={{ fontSize: "14px", fontWeight: 600, color: "#1C1C1C" }}>{r.title || r.description || "Health Record"}</p>
				...
				{records.length === 0 && (
					<div className="text-center py-8">
						<p style={{ fontSize: "14px", color: "#888" }}>No health records found for this pet.</p>
					</div>
				)}

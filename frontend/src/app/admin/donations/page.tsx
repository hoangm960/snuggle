...
const thisMonth = donations
	.filter((d) => d.date.startsWith("2025-12") && d.status === "Completed")
	.reduce((s, d) => s + d.amount, 0);
const avgDonation = Math.round(
	totalRaised / donations.filter((d) => d.status === "Completed").length
);
...
export default function DonationsPage() {
	...
	return (
		<AdminLayout
			title="Donations"
			subtitle="Track contributions and financial support for Snuggle's mission."
		>
			{/* Stats */}
						...
						<ResponsiveContainer width="100%" height="100%">
							<PieChart>
								<Pie
									...
									paddingAngle={3}
								>
									{donationByCategory.map((entry, i) => (
			...
			<div className="bg-card border border-border rounded-3xl p-6 shadow-card mb-8">
				<div className="flex items-center justify-between mb-6">
					<div>
						...
						<p className="text-xs text-muted-foreground">
							Monthly breakdown — one-time, recurring &amp; annual
						</p>
					</div>
						...
						<tbody className="divide-y divide-border">
							{filtered.map((d) => (
								<tr key={d.id} className="hover:bg-secondary/30 transition-colors">
									...
									<td className="px-6 py-4">
										<span
											className={`px-2.5 py-1 rounded-full text-[11px] font-semibold ${typeBadge[d.type]}`}
										>
											{d.type}
										</span>
									</td>
									...
									<td className="px-6 py-4">
										<div className="flex items-center gap-2">
											<div
												className={`size-2 rounded-full ${statusDot[d.status]}`}
											/>
											<span
												className={`text-xs font-medium ${statusText[d.status]}`}
											>
												{d.status}
											</span>
										</div>

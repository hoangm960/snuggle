...
export default function RequestsPage() {
  ...
  return (
    <AdminLayout title="Adoption Requests" subtitle="Review applications, schedule visits and approve forever homes.">
      {/* Tabs */}
        ...
        {(["All", "Pending", "Reviewing", "Approved", "Delivered"] as const).map((t) => (
          <button
            ...
            className={`px-5 h-11 rounded-full text-sm font-semibold whitespace-nowrap flex items-center gap-2 transition-colors ${
              tab === t ? "bg-primary text-primary-foreground shadow-glow" : "bg-card border border-border text-muted-foreground hover:text-foreground"
            }`}
          >
            {t}
            <span className={`text-[11px] px-2 py-0.5 rounded-full ${tab === t ? "bg-primary-foreground/20" : "bg-secondary"}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        {filtered.map((r) => (
          <article key={r.id} className="bg-card border border-border rounded-3xl p-6 shadow-card hover:shadow-soft transition-shadow">
            <div className="flex items-start justify-between mb-4">
              <span className="text-[10px] font-mono text-muted-foreground">{r.id}</span>
              <span className={`px-3 py-1 rounded-full text-[11px] font-semibold ${statusBadge[r.status]}`}>{r.status}</span>
            </div>

            <div className="flex gap-4 items-center mb-5">
              <img src={r.petImage} alt={r.petName} className="size-16 rounded-2xl object-cover" />
              ...
              <div className="text-right">
                <p className="text-[10px] uppercase text-muted-foreground tracking-widest">Submitted</p>
            ...
            <div className="flex items-center gap-3 p-4 rounded-2xl bg-secondary/40 mb-4">
              <img src={r.applicantAvatar} alt={r.applicantName} className="size-10 rounded-full object-cover" />

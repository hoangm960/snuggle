...
export function EditShelterModal({ isOpen, onClose, onSuccess, shelter }: EditShelterModalProps) {
	const [name, setName] = useState(shelter.name);
	const [address, setAddress] = useState(shelter.address);
	const [contactEmail, setContactEmail] = useState(shelter.contactEmail);
	const [phone, setPhone] = useState(shelter.phone || "");
	const [description, setDescription] = useState(shelter.description || "");
	const [adminUserId, setAdminUserId] = useState(shelter.adminUserId);
	...
	useEffect(() => {
		if (isOpen) {
			...
			setAdminUserId(shelter.adminUserId);
		}
	}, [isOpen, fetchUsers, shelter]);
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			await api.put(`/admin/shelters/${shelter.id}`, {
				name,
				address,
				contactEmail,
				phone,
			...
			setError(err.response?.data?.message || "Failed to update shelter");
		} finally {
		...
		<div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-0">
			...
			<div className="relative w-full max-w-lg bg-white rounded-2xl shadow-xl flex flex-col max-h-[90vh]">
				<div className="flex items-center justify-between p-6 border-b border-gray-100 shrink-0">
					<div>
						<h2 className="text-xl font-semibold text-gray-900">Edit Shelter</h2>
						<p className="text-sm text-gray-500 mt-1">
							Update shelter details and assignment.
						</p>
					</div>
					<button
						onClick={onClose}
				...
				<div className="p-6 overflow-y-auto">
					...
					<form id="edit-shelter-form" onSubmit={handleSubmit} className="space-y-4">
						<div>
